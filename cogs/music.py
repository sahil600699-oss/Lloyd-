...
class Music(commands.Cog):
    ...
    async def on_wavelink_track_end(self, payload: wavelink.TrackEndEventPayload):
        ...
        # Ignore the track-end event caused by a manual !stop/button stop.
        if state.manual_stop:
            state.manual_stop = False
            return

        # Ignore a delayed end event from the previous track after a skip.
        # Without this guard, the old track-end event can race with skip_track()
        # and remove/update the music panel for the newly started song.
        ended_track = getattr(payload, "track", None)
        if ended_track is not None and state.current is not None and ended_track is not state.current:
            return
        ...
        if state.queue:
            ...
            await self.update_panel(player.guild)
        else:
            state.current = None
    ...
    async def make_embed(self, track, state, paused=False):
        ...
        embed.add_field(name="Volume", value=f"**{state.volume}%**", inline=False)
        embed.add_field(
            name="Duration",
            value=f"**{format_duration(getattr(track, 'length', 0))}**",
            inline=False,
        )
    ...
    async def update_panel(self, guild, view=None):
        ...
        if not message or not state.current:
            return
    ...
    async def send_now_playing(self, channel, track, state):
        ...
        # Keep ONE music panel message and edit it when the next song starts.
        if state.panel_message:
            try:
